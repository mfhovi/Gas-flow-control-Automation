...
This stand alone program is developed for Mass flow controller (GMC1200) for gas flow automation/scheduling
...
3) When appropriate connector is pluged into the system (e.g. PC), the active COM port can viewed from "Device Manager".
...
4) From the flow controller, control mode has to be changed to RS232 by MENU->Control Mode->RS232->Ent.

5) There are 2 python files, one for two channel flow and another for multichannel flow control, they can be run directly with python. Otherwise, after the python files are downloaded or copied in local drive (e.g. C:\Downloads) the following command has to be given in Command Prompt to get windows executable program,
...
6) After selecting appropriate COM port and Baud rate (9600 by default), instruction can be done with the GMC1200
